...
export default class Model {
              ...
              await this.docRef
                ...
                .then(_objs => {
                  if (
                    ...
                    const objIds = Object.keys(_objs.val()).map(objId => {
                      if (_objs.val()[objId][key] === sort[key]) {
                        return objId;
                      }
                    });
  ...
  async create(info) {
    return new Promise(async (resolve, reject) => {
      const objPath = this.docRef.push();

      await objPath
        .set({ ...info, id: objPath.key, originalId: info.id })
        .then(async () => {
          await this.get({ id: objPath.key })
  ...
  async getOrCreate(identifier, info) {
    return new Promise(async (resolve, reject) => {
